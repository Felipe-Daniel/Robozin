...
import robozin.recommenders.jayes.factor.opcache.IOperationCache;
import robozin.recommenders.jayes.factor.opcache.ModuloCache;
...
public abstract class AbstractFactor implements Cloneable {
    ...
    /**
     * tells the Factor which variables the dimensions correspond to. Uniqueness and
     * consistency of size is assumed.
     */
    ...
    /**
     * multiply the factors. Only compatible factors are allowed, meaning ones that
     * have a subset of the variables of
    ...
    private void sumPreparedLog(Cut cut, int offset, IArrayWrapper compatibleFactorValues, int[] positions,
            double max) {
        if (cut.getSubCut() == null) {
            int last = cut.getEnd() + offset;
            for (int i = cut.getStart() + offset; i < last; i += cut.getStepSize()) {
